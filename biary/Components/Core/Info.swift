...
//친구 정보 불러올 때
extension Info {
    static var null: Info {
        return Info(user: nil, books: [], contents: [])
    }
    ...
    static func make(data: JSON) -> Info {
        ...
        var temp = Info(user: User.transformUser(fromJSON: json["user"]),
                        books: Book.transformBooks(fromJSON: json["books"]),
        ...
        for i in privatebooks {
            guard let a = temp.user else {return Info.null}
            temp.user!.bookShelf = Bookshelf.clean(from: a.bookShelf , bookToken: i.token)
        } 
